...
def detect_face(img, minsize, pnet, rnet, onet, threshold, factor):
    ...
    if numbox > 0:
        ...
        points[0:5, :] = np.add(np.multiply(np.tile(
            w, (5, 1)), points[0:5, :]), np.subtract(np.tile(total_boxes[:, 0], (5, 1)), 1))
        points[5:10, :] = np.add(np.multiply(np.tile(
            h, (5, 1)), points[5:10, :]), np.subtract(np.tile(total_boxes[:, 1], (5, 1)), 1))
        if total_boxes.shape[0] > 0:
...
def bbreg(boundingbox, reg):
    ...
    b2 = np.add(boundingbox[:, 1],np.multiply(reg[:, 1], h))
    b3 = np.add(boundingbox[:, 2],np.multiply(reg[:, 2], w))
    b4 = np.add(boundingbox[:, 3],np.multiply(reg[:, 3], h))
    boundingbox[:, 0:4] = np.transpose(np.vstack([b1, b2, b3, b4]))
    return boundingbox
...
def generateBoundingBox(imap, reg, scale, t):
    ...
    q2 = np.fix(np.divide(np.add(np.multiply(stride, bb),
                                 (np.add(np.subtract(cellsize, 1), 1))), scale))
